...
def train(output, name):
    md: LightFM = load(os.path.join(output, name, "model"))
    valid = load(os.path.join(output, name, "valid_data"))
    train = load(os.path.join(output, name, "train_data"))
    sm_user_features = load(os.path.join(output, name, "user_features"))
    sm_item_features = load(os.path.join(output, name, "item_features"))
    ...
    save_json({"model": name, "metrics": {"AUC": str(auc), "MAP@K": str(mapk)}}, os.path.join(output, name, "metrics"))
